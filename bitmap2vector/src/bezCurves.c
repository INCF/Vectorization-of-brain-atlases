...
{   
    ...
    numPoints = (int *)malloc(numPaths * sizeof(int));

    for(i = 0; i < numPaths; ++i)
    {
        scanf("%d", &numPoints[i]);
        d[i] = (Point2*)malloc(numPoints[i] * sizeof(Point2));
        for(j = 0; j < numPoints[i]; ++j)
        {
            scanf("%lf%lf", &d[i][j].x, &d[i][j].y);
        }
        printf("curve(%d).path = [", i+1);
        FitCurve(d[i], numPoints[i], error);           /*  Fit the Bezier curves */
        printf("];\n");
    }
