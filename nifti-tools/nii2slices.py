...
def run(args):
  ...
  try:
    ...
    for i in range(0,numSlices):
      ...
      imgFile = baseName+'_{:04d}.{}'.format(i,args.format)
      rgbImg = rgbImg.reshape(shape[0],shape[1],3)
      if args.format is 'png':
        scipy.misc.toimage(rgbImg).save(op.join(pngFolder,imgFile), optimize=True)
      elif args.format is 'jpg':
        scipy.misc.toimage(rgbImg).save(op.join(pngFolder,imgFile), quality=80, optimize=True, progressive=True)
      print 'image {} saved to file "{}".'.format(i,pngFile)
